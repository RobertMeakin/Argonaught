...
namespace Argonaught.Internal
{
    ...
    {
        internal static IRequestType GetArgonaughtRequestType(this HttpContext context, string apiPath)
        {
            ...
            {
                var pascalGrantType =
                    (from x in context.Request.Form["grant_type"].ToString().Split('_')
                ...
                requestType = (from at in ArgonautRequestTypes.GetTypes()
                               let t = at.GetType()
                               where t.Name.Substring(0, t.Name.Replace("RequestType", "").Length).Equals(pascalGrantType)
